...
const productAssetsStorage = multer.diskStorage({
  destination: (_req, file, cb) => {
    cb(null, file.fieldname === "image" ? IMAGES_DIR : FILES_DIR);
  },
  filename: (_req, file, cb) => cb(null, buildFilename(file)),
});

function productAssetsFileFilter(
  _req: Request,
  file: Express.Multer.File,
  cb: FileFilterCallback
): void {
...
export const uploadProductAssets = multer({
  storage: productAssetsStorage,
  fileFilter: productAssetsFileFilter,
  limits: { fileSize: MAX_PRODUCT_FILE_SIZE },
}).fields([
